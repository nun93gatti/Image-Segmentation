...
def convblock(cdim, nb, bits=3):
	...
	for k in range(1, bits + 1):
		...
		if False:
			...
			L.append(ZeroPadding2D((1, 1),data_format='channels_last'))
			L.append(Convolution2D(cdim, kernel_size=(3, 3), activation='relu', name=convname, data_format='channels_last'))
		else:
			L.append(Convolution2D(cdim, kernel_size=(3, 3), padding='same', activation='relu', name=convname, data_format='channels_last'))

	L.append(MaxPooling2D(pool_size=(2, 2), strides=(2, 2), data_format='channels_last'))

	return L


def fcn32_blank(image_size):
